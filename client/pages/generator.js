      ...
      <div class="card animate-in" id="gen-form-card">
        ...
        <div class="form-group">
          ...
          <select class="form-select" id="doc-type">
            ...
            <option value="data_retention_policy">▤ Data Retention Policy</option>
            <option value="incident_response_plan">▤ Incident Response Plan</option>
          </select>
        </div>
        <div class="form-group">
          <label class="form-label">Company *</label>
          <input type="text" class="form-input" id="gen-company-name" placeholder="Company name" />
        </div>
        <div class="form-row">
          <div class="form-group">
            <label class="form-label">Industry</label>
            <select class="form-select" id="gen-industry">
              <option value="">Select...</option>
          ...
          <div class="form-group">
            ...
            <select class="form-select" id="gen-country">
              <option value="">Select...</option>
        ...
        <div class="form-group">
          ...
          <select class="form-select" id="gen-regulation">
            <option value="">Select...</option>
        ...
        <button class="btn btn-primary btn-lg" style="width:100%;margin-top:var(--sp-3);" id="btn-generate">
          ◆ GENERATE
        </button>
        <p style="margin-top:var(--sp-3);font-family:var(--font-mono);font-size:9px;color:var(--text-tertiary);">
          [!] templates only. review with legal counsel.
        </p>
      </div>

      <div class="animate-in" id="gen-preview">
        <div class="empty-state" style="margin-top:var(--sp-10);">
          <div style="font-size:48px;opacity:0.15;margin-bottom:var(--sp-3);">▤</div>
          <h3>Document Preview</h3>
          <p>> configure and generate to preview</p>
        </div>
      </div>
    </div>
  `;
...
async function handleGenerate() {
  ...
  preview.innerHTML = `<div class="loading-container"><div class="spinner"></div><div class="loading-text">> drafting document...</div><div class="loading-subtext">~10-15 seconds</div></div>`;
...
function renderDocPreview(container, doc) {
  ...
  container.innerHTML = `
    <div class="doc-preview">
      <div class="doc-preview-header">
        <div>
          <div class="doc-preview-title">${doc.title || 'Document'}</div>
          <div style="font-family:var(--font-mono);font-size:9px;color:var(--text-tertiary);margin-top:2px;">${doc.company || ''} · ${doc.regulation || ''} · ${doc.last_updated || ''}</div>
        </div>
        <div class="doc-preview-actions">
          <button class="btn btn-ghost" id="btn-copy-doc">COPY</button>
          <button class="btn btn-ghost" id="btn-dl-doc">↓ SAVE</button>
        </div>
      </div>
      <div class="doc-preview-body">
        ${sections.map(s => `<div class="doc-section"><h3>${s.heading}</h3><p>${s.content}</p></div>`).join('')}
      </div>
      ${doc.disclaimer ? `<div class="doc-disclaimer">[!] ${doc.disclaimer}</div>` : ''}
    </div>
  `;
  ...
  document.getElementById('btn-dl-doc')?.addEventListener('click', () => {
    ...
    a.href = url;
    a.download = `${(doc.title || 'doc').replace(/\s+/g, '-').toLowerCase()}.txt`;
    a.click();
