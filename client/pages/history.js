import { getScans, getScanById as getScanDetail } from '../services/supabase.js';

export function renderHistory(container, user) {
  container.innerHTML = `
    <div class="page-header animate-in"><h1>History</h1><p>> past compliance scan records</p></div>
...
async function loadHistory(user) {
  ...
  try {
    ...
    card.innerHTML = `<table class="history-table"><thead><tr><th>Company</th><th>Regulation</th><th>Risk</th><th>Items</th><th>Date</th><th></th></tr></thead><tbody>
      ${scans.map(s => {
      ...
      return `<tr><td style="font-weight:700;color:var(--text-primary);">${s.company_name}</td><td>${s.regulation}</td><td><span class="badge badge-${rc}">${s.overall_risk}</span></td><td>${findingsCount}</td><td style="color:var(--text-tertiary);font-size:10px;">${date}</td><td><button class="btn btn-ghost btn-view" data-id="${s.id}">VIEW →</button></td></tr>`;
    }).join('')}</tbody></table>`;
...
async function viewDetail(id) {
  ...
  try {
    const r = await getScanDetail(id);
    const rc = { HIGH: 'red', MEDIUM: 'amber', LOW: 'green' };
    const sc = { COMPLIANT: 'green', PARTIALLY_COMPLIANT: 'amber', NON_COMPLIANT: 'red' };
    const findings = r.findings || [];
    detail.innerHTML = `<div class="card">
      <div class="card-header"><div><h2 class="card-title">${r.company_name} — ${r.regulation}</h2><div class="card-subtitle">Risk: <span class="badge badge-${rc[r.overall_risk]}">${r.overall_risk}</span></div></div>
      ...
      return `<div class="finding-card" onclick="this.classList.toggle('expanded')"><div class="finding-header"><span class="finding-title">${f.requirement}</span><div class="finding-meta"><span class="badge badge-${sc[f.status]}">${f.status?.replace('_', ' ')}</span><span class="badge badge-${rc[f.risk_level]}">${f.risk_level}</span></div></div>
